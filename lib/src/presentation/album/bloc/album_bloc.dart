...
class AlbumBloc extends Bloc<AlbumEvent, AlbumState> {
  ...
  void _loadProductionAlbums(
    ...
    emit(AlbumLoading());
    final data =
        await iAlbumRepository.getProductionAlbums(event.pId, event.page);
    data.fold((l) {
      if (l is ServerFailure) {
        emit(AlbumLoadFailed(l.message ?? ""));
      }
  ...
  void _loadArtistAlbums(
    ...
    emit(AlbumLoading());
    final data =
        await iAlbumRepository.getArtistAlbums(event.artistId, event.page);
    data.fold((l) {
      if (l is ServerFailure) {
        emit(AlbumLoadFailed(l.message ?? ""));
      }
  ...
  void _loadAllAlbum(GetAllAlbumEvent event, Emitter<AlbumState> emit) async {
    emit(AlbumLoading());
