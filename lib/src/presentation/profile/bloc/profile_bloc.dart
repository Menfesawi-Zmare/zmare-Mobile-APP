...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  void _loadSubsriptions(
    ...
    emit(ProfileLoading());
    final data =
        await iProfileRepository.getSubscriptions(event.profileId, event.page);
    data.fold((l) {
  ...
  void _loadSubscribers(
    ...
    emit(ProfileLoading());
    final data =
        await iProfileRepository.getSubscribers(event.profileId, event.page);
    data.fold((l) {
      if (l is ServerFailure) {
        emit(ProfileFailed(l.message ?? ""));
      }
  ...
  void _loadPlaylists(
    ...
    emit(ProfileLoading());
    final data =
        await iProfileRepository.getPlaylists(event.profileId, event.page);
    data.fold((l) {
      if (l is ServerFailure) {
        emit(ProfileFailed(l.message ?? "No Data"));
      }
