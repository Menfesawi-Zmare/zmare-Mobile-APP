...
class SearchRepositoryImpl extends ISearchRepository {
  ...
  Future<Either<Failure, TrackList>> fetchSongSearchResults(
    ...
    final response =
        await iSearchDataSources.fetchSongSearchResults(query, page);
  ...
  Future<Either<Failure, AlbumList>> fetchAlbumSearchResults(
      String query, int page) async {
    final response =
        await iSearchDataSources.fetchAlbumSearchResults(query, page);
    return response.fold(
      (failure) => Left(failure),
      (albumsResponse) {
  ...
  Future<Either<Failure, ArtistList>> fetchArtistSearchResults(
      String query, int page) async {
    final response =
        await iSearchDataSources.fetchArtistSearchResults(query, page);
    return response.fold(
      (failure) => Left(failure),
      (artistsResponse) {
  ...
  Future<Either<Failure, PlaylistList>> fetchPlaylistSearchResults(
      String query, int page) async {
    final response =
        await iSearchDataSources.fetchPlaylistSearchResults(query, page);
    return response.fold(
      (failure) => Left(failure),
      (playlistsResponse) {
  ...
  Future<Either<Failure, ProfileList>> fetchProfileSearchResults(
      String query, int page) async {
    final response =
        await iSearchDataSources.fetchProfileSearchResults(query, page);
    return response.fold(
      (failure) => Left(failure),
      (profilesResponse) {
  ...
  Future<Either<Failure, SuggestionList>> fetchSearchSuggestionResults(
      String query) async {
    final response =
        await iSearchDataSources.fetchSearchSuggestionResults(query);
    return response.fold(
      (failure) => Left(failure),
      (suggestionResponse) {
