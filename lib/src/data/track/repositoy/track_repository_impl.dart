...
import 'package:zmare/src/data/track/data_sources/track_data_source.dart';
...
class TrackRepositoryImpl extends ITrackRepository {
  ...
  Future<Either<Failure, CommentResponse>> editTrackComment(
    ...
    final response =
        await iTrackDataSource.editTrackComment(commentId, comment);
    return response.fold(
      (failure) => Left(failure),
      (comment) {
        return Right(comment);
  ...
  Future<Either<Failure, bool>> trackReport(
    ...
    final response =
        await iTrackDataSource.trackReport(trackReportRequestModel, trackId);
    return response.fold(
      (failure) => Left(failure),
      (report) {
        return Right(report);
