...
Future<void> removeLiked(String key) async {
  final Box likedBox = Hive.box(BoxType.favorite.name);
  likedBox.delete(key);
  // setState(() {});
}

Future<void> addMapToPlaylist(String name, Map info) async {
  if (name != BoxType.favorite.name) await Hive.openBox(name);
...
Future<void> addPlaylist(String inputName, List data) async {
  ...
  final List playlistNames = Hive.box(BoxType.playlists.name)
      .get('playlistNames', defaultValue: []) as List;
  ...
  Hive.box(BoxType.playlists.name).put('playlistNames', playlistNames);
}
